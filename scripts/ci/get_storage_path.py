...
def main():
    if len(sys.argv) < 4:
        print("Usage: python get_storage_path.py <type> <version> <filename> [channel]", file=sys.stderr)
    ...
    try:
        if path_type == "path":
            ...
            if not cdn_base:
                print("Error: SPACES_CDN_BASE environment variable not set", file=sys.stderr)
                sys.exit(1)
